...
#Based on https://github.com/kernelkaribou/synology-monitoring
#by kernelkaribou

#Most of the code is unchanged, instead of sending everything to influxDB
#It creates a JSON file you can just fetch for static webpages
#set the output path as first arguement when running the script ex: "bash /var/scripts/syno_json.sh /var/web/syno.json" or w/e
...
capture_ups="true" #get UPS status such as runtime, status and charge level. Default false as it will fail if no UPS is configured.
...
while [ $i -lt $total_executions ]; do
	...
	if (${capture_system,,} = "true"); then
		...
		#Fan status, 1 is online, 0 is failed/offline
		...
		json=$json"\"$measurement\":{\"nas_name\":\"$nas_name\", \"uptime\":$system_uptime,\"system_status\":$system_status,\"fan_status\":$system_fan_status,\"model\":$model,\"serial_number\":$serial,\"upgrade_status\":$upgrade,\"dsm_version\":$version,\"system_temp\":$system_temp},
"
	else
	...
	if (${capture_memory,,} = "true"); then
		...
		json=$json"\"$measurement\":{\"nmem_total\":$mem_total,\"mem_avail_real\":$mem_avail_real,\"mem_buffer\":$mem_buffer,\"mem_cached\":$mem_cached,\"mem_free\":$mem_free},
"
	else
	...
	if (${capture_cpu,,} = "true"); then
		...
		json=$json"\"$measurement\":{\"usage_idle\":$usage_idle},
"
	else
	...
	if (${capture_volume,,} = "true"); then
		...
		while IFS= read -r line; do
			...
			if [[ $line =~ "/volume"+([0-9]$) ]]; then
				...
				json=$json"\"$measurement\":{\"volume\":\"$vol_name\",\"vol_totalsize\":$vol_totalsize,\"vol_used\":$vol_used},
"
	...
	if (${capture_raid,,} = "true"); then
		...
		do
			...
			json=$json"{\"raid_name\":\"$raid_name\",\"raid_status\":$raid_status,\"raid_free_size\":$raid_free_size,\"raid_total_size\":$raid_total_size},
"
		done
		json=${json%??}"],"
	else
	...
	if (${capture_disk,,} = "true"); then
		...
		do
			...
			json=$json"{\"disk_name\":\"$disk_name\",\"disk_model\":$disk_model,\"disk_type\":$disk_type,\"disk_temp\":$disk_temp,\"disk_status\":$disk_status},
"
		...
		do
			...
			json=$json"{\"disk_path\":\"$disk_path\",\"disk_reads\":$disk_reads,\"disk_writes\":$disk_writes,\"disk_load\":$disk_load},
"
	...
	if (${capture_ups,,} = "true"); then
		...
		json=$json"\"$measurement\":{\"ups_group\":\"$ups_group\",\"ups_status\":$ups_status,\"ups_load\":$ups_load,\"ups_battery_runtime\":$ups_battery_runtime,\"ups_battery_charge\":$ups_battery_charge},
"
	else
	...
	if (${capture_network,,} = "true"); then
		...
		do
			...
			json=$json"{\"interface_name\":\"$interface_name\",\"bytes_recv\":$bytes_recv,\"bytes_sent\":$bytes_sent},
"	
		done
		json=${json%??}"],"
	else
